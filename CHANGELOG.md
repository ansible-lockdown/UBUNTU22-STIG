...
- new CI, both secret-free and running in front of the existing pipelines: `.github/workflows/molecule.yml` (container smoke test, the `default` scenario) and `.github/workflows/repo_qa.yml` (the Ansible-Lockdown QA checker pinned to `2.8.3`, run with `--strict` against `.qa_baseline.json`)
